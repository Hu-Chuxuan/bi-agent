...
from .bi_lib.py_tools_gt import execute_tool as execute_py_tool
from .bi_lib.sql_tools_gt import execute_tool as execute_sql_tool
from .bi_lib.prompt import (
    PROMPT_PY, PROMPT_PY_TOOL,
    PROMPT_SQL, PROMPT_SQL_TOOL,
    TOOLS_PY_BASELINE, TOOLS_PY,
    TOOLS_SQL_BASELINE, TOOLS_SQL,
...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        _VALID_MODES = {"py_no_tool", "py_tool", "sql_no_tool", "sql_tool"}
        self.bi_mode = os.environ.get("BI_MODE", "py_no_tool")
        if self.bi_mode not in _VALID_MODES:
            raise ValueError(f"BI_MODE must be one of {_VALID_MODES}, got '{self.bi_mode}'")

        _D = os.environ.get("RFT_DATA_DIR")
        _MODE_CONFIG = {
            "py_no_tool":  {"prompt": PROMPT_PY,      "tools": TOOLS_PY_BASELINE},
            "py_tool":     {"prompt": PROMPT_PY_TOOL,  "tools": TOOLS_PY},
            "sql_no_tool": {"prompt": PROMPT_SQL,      "tools": TOOLS_SQL_BASELINE},
            "sql_tool":    {"prompt": PROMPT_SQL_TOOL,  "tools": TOOLS_SQL},
        }
        cfg = _MODE_CONFIG[self.bi_mode]
        self._cases_path      = f"{_D}/{self.bi_mode}/training_case_ids.json"
        self._gt_path         = f"{_D}/{self.bi_mode}/gt"
        self._prompt_template = cfg["prompt"]
        self._tools_template  = cfg["tools"]
    ...
    def _generate_and_score_completions(self, inputs: DataType) -> DataType:
        ...
        rewards, temp = [], []
        is_py  = self.bi_mode.startswith("py")
        use_tool = self.bi_mode.endswith("tool")

        for _ in range(self.num_generations):
            folder = inputs[0]["messages"][0]["content"]
            prompt, system_prompt, input_messages = None, None, []

            # -------------------
            # Rank 0 prepares query + DB
            # -------------------
            if self.accelerator.is_main_process:
                ...
                csv_list = [
                    f for f in glob.glob(f"{temp_folder}/*.csv")
                    if not os.path.basename(f).startswith("_")
                ]

                if is_py:
                    data_list = csv_list
                    conn, db_path = None, None
                else:
                    df_list, data_list = {}, []
                    for _csv in csv_list:
                        try:
                            ...
                            df = pd.read_csv(
                                _csv, low_memory=False, dtype=str,
                            ...
                            pass
                    try:
                        db_path = f"{os.environ.get('RFT_DATA_DIR')}/temp/sqlite_db_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.db"
                ...
                prompt = self._prompt_template.format(query=query, all_files=data_list)
                system_prompt = DEFAULT_SYSTEM_PROMPT + str(self._tools_template + FUNCTION_STOPPER)
            ...
            # -------------------
            [input_messages] = broadcast_object_list([input_messages], from_process=0)

            # -------------------
            # All ranks must call generate
            # -------------------
            max_retries, i = 10, 0
            result, state = None, {}
            tool_call_seq = []
            ...
            while i < max_retries:
                i += 1
                engine_output = self._generate_completions([{"messages": input_messages}])

                stop, responses = False, []
                if self.accelerator.is_main_process:
                    message = engine_output[0]["messages"][-1]["content"]
                    print(message)

                    if is_py:
                        user_message = re.sub(r'<think>.*?</think>', '', message, flags=re.DOTALL)
                        tool_calls = parse_tool_call_string_python(user_message)
                    else:
                    ...
                    if not tool_calls:
                        ...
                        if is_py:
                            ...
                            match = re.search(json_pattern, user_message)
                            if match:
                                tool_call_block = f"```json\n{match.group(1)}```"
                            else:
                                tool_call_block = f"```json\n{extract_first_json(user_message)}```"
                            update_input_messages.append({"role": "assistant", "content": tool_call_block})

                        for tool_call in tool_calls:
                            if tool_call.function.name == "stopper":
                                stop = True
                                break
                            tool_call_seq.append(tool_call)
                            ...
                            try:
                                ...
                                if is_py and tool_call.function.name == "retrieve_relevant_tables":
                                    relevant_tables = output
                                if result_dict.get("tool_name") == "execute_code":
                                    ...
                                    res_result = json.dumps(
                                        {"result (first 5 rows)": tmp_res_str, "outputs": output}
                                    )
                                ...
                                res_result = json.dumps({"outputs": f"[ERROR] {tb}"})
                            responses.append(res_result)

                        if is_py and "SyntaxError" in str(responses):
                            num_syntax_error += 1

                        input_messages.append(
                            {"role": "user", "content": f"The responses of {message} is {str(responses)}"}
                        )
                        if is_py:
                            update_input_messages.append(
                                {"role": "user", "content": f"The responses of {tool_call_block} is {str(responses)}"}
                            )
                            print(responses)

                        # tool-sequence validation (py_tool only)
                        if is_py and use_tool:
                            if len(tool_call_seq) == 1 and (tool_call_seq[0].function.name != "transform_tables" or i != 1):
                                following_tool = False
                            if len(tool_call_seq) == 2 and (tool_call_seq[1].function.name != "retrieve_relevant_tables" or i != 2):
                                following_tool = False
                            if len(tool_call_seq) == 3:
                                if i != 3:
                                    following_tool = False
                                if tool_call_seq[0].function.name != "transform_tables" \
                                or tool_call_seq[1].function.name != "retrieve_relevant_tables" \
                                or tool_call_seq[2].function.name != "discover_join_relationships" \
                                or not set(relevant_tables).issubset(
                                    set(json.loads(tool_call_seq[2].function.arguments).get("table_list", []))
                                ):
                                    following_tool = False

                # --- broadcast stop + responses so all ranks agree ---
                [stop, responses, input_messages] = broadcast_object_list(
                    [stop, responses, input_messages], from_process=0
                )

                if stop:
                    if not is_py and input_messages and input_messages[-1]["role"] == "user":
                        input_messages.pop()
                    break

            # -------------------
            # Rank 0: finalize reward
            # -------------------
            if self.accelerator.is_main_process:
                if not is_py and conn:
                    conn.close()
                ...
                if result is not None:
                    ...
                    gt_pattern = f"{self._gt_path}/{folder}_*.csv"
                    file_paths = glob.glob(gt_pattern)
                    gt = [
                        pd.read_csv(f)
                        for f in (file_paths or [f"{self._gt_path}/{folder}.csv"])
                    ]
                    print("*********result: ")
                    print(result)
                    print("*********gt: ")
                    ...
                    try:
                        ...
                        if is_py:
                            reward = [1. - 0.1 * num_syntax_error] if final_res else [-0.5 - 0.1 * num_syntax_error]
                        else:
                            reward = [1.] if final_res else [-0.5]
                    except Exception:
                        final_res = False
                        reward = [-0.5 - 0.1 * num_syntax_error] if is_py else [-0.5]
                else:
                    final_res = False
                    reward = [-1. - 0.1 * num_syntax_error] if is_py else [-1.]

                print("*****final res:")
                print(final_res)
                if is_py:
                    print("*****following tool:")
                    print(following_tool)
                    print("*****number of syntax errors:")
                    print(num_syntax_error)
                    input_messages = update_input_messages
                    print(input_messages)
            else:
                reward = None

            # --- broadcast reward so all ranks append the same thing ---
            if is_py:
                [reward, input_messages] = broadcast_object_list([reward, input_messages], from_process=0)
            else:
                [reward] = broadcast_object_list([reward], from_process=0)
            rewards.append(reward)

            # engine_output is already identical on all ranks → safe to just extend
            temp.extend(engine_output)

        # -------------------
        # Sync + advantages
        # -------------------
        self.accelerator.wait_for_everyone()
        total_rewards_per_func = torch.tensor(rewards).to(self.accelerator.device)
        inputs = temp
        ...
        if self.args.dynamic_sample and mode == "train":
            inputs, total_rewards_per_func = self._dynamic_sampling(inputs, total_rewards_per_func)

        total_advantages = self._compute_advantages(inputs, total_rewards_per_func)
        assert len(total_advantages) == len(inputs)
        for i, advantage in enumerate(total_advantages):
            ...
            with patch_profiling_context(self, 'log_metrics'):
                ...
                if metrics_for_logs_to_gather:
                    for key, value in metrics_for_logs_to_gather.items():
                        ...
                        self._logs[key].extend(value, flatten_level=0)
