...
def generate_from_folder(id, model, transform_method="join"):
    ...
    for i, comp in enumerate(max_components):  # comp: {'fact_table': str, 'path': List[str]}
        ...
        query = None
        code = None
        while (len(table) < 2 or fact_table not in table or gt is None or not isinstance(gt, pd.DataFrame) or gt.empty or gt.dropna(how="all").empty) and max_retries > 0:
...
if __name__ == "__main__":
    ...
    generate_from_folder(args.id, args.model)