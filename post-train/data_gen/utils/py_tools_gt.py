...
def execute_tool(tool_call, queries, id, state, temp_folder, csv_list, transform_method):
    ...
    if tool_name == "retrieve_relevant_tables":
        ...
        execution_res = tool_fn(id, temp_folder, table_list, transform_method)
    else:
...
def transform_tables(queries, id, temp_folder):
    ...
    for transformation_name, table_list in transformations.items():
        ...
        for table_name in table_list:
            ...
            transformed_path = f"./cases_{transformation_name}/transformation/{id}/{transformation_name}/{table_name}"
...
def _make_picklable(d: dict) -> dict:
    """Keep values that pickle can handle. Fallback to repr for the rest."""
    clean = {}
    for k, v in d.items():
        try:
            pickle.dumps(v)      # quick test
            clean[k] = v
        except Exception:
            clean[k] = repr(v)
    return clean
...
def _exec_code(code: str, state: dict, out_q: mp.Queue) -> None:
    ...
    try:
        ...
        payload = {
            ...
            "state": _make_picklable(safe_globals),
        }
    except Exception:
        payload = {
            "code": code,
            ...
            "state": _make_picklable(safe_globals if 'safe_globals' in locals() else {}),
        }
...
def execute_code(code: str, state: dict | None = None, timeout: int = TIMEOUT) -> dict:
    out_q = mp.Queue()
    ...
    try:
        ...
        result = {
            "code": code,
            "result": None,
            "outputs": f"[ERROR] execution timed out after {timeout} seconds.",
            "state": state or {}
        }
...
def discover_join_relationships(id, temp_folder, table_list, transform_method):
