...
# ---------------------------------------------------------------------------
# Open-source model: text-based tool-call parsing
# ---------------------------------------------------------------------------
...
def _call_model(model: str, messages, tools_spec):
...
def _run_tool_call(tc, language, queries, folder_id, conn, temp_folder, csv_list, data_list, state):
    ...
    try:
        if language == "sql":
            result_dict = execute_sql_tool(tc, queries, folder_id, conn, temp_folder, data_list)
        else:
            exec(
                "import pandas as pd\n"
            ...
            result_dict = execute_py_tool(tc, queries, folder_id, state, temp_folder, csv_list)
...
def main(folder_id: str, model: str, language: str, tool: bool, log_path: str):
    ...
    for i in range(1, 31):
        response = _call_model(model, input_messages, tools_spec)

        with open(log_path, "a") as lf:
            lf.write(f"========Runtime {i}========\n")
            lf.write(f"Input Messages:\n{input_messages}\nRaw Response:\n{response.model_dump()}\n\n")

        # ---- Open-source branch: text-based tool call parsing ----
        if is_open_source(model):
            ...
            input_messages.append({"role": "tool", "content": str(responses)})
        ...
        # ---- Proprietary branch: structured tool calls ----
        else:
            ...
            with open(log_path, "a") as lf:
                ...
                lf.write(f"state: {str({k: v for k, v in state.items() if not k.startswith('__')})}\n\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", required=True, choices=sorted(ALL_MODELS), help="Model name")
    ...
    parser.add_argument("--log-csv", default="results.csv", help="CSV file to append summary results")
    parser.add_argument("--log-dir", default="logs", help="Base directory for log files")
    args = parser.parse_args()

    run_tag = f"{args.model}_{args.language}_{args.tool}_{args.id}"
    ...
    for i in range(args.runs):
        ...
        final_res, tool_call_seq = main(args.id, args.model, args.language, args.tool, log_path)
